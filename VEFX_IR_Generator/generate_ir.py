...
def _graphiceq_points(effect_type, low_eq, high_eq, filter_slider):
    """temp_file[14]: 4-point GraphicEQ -> (g_low, g_160, g_2500, g_hi) dB."""
    f, lo, hi = _filter_db(filter_slider), _eq_db(low_eq), _eq_db(high_eq)
    return (f, lo, hi, f) if effect_type in (1, 2) else (f + 6, lo + 3, hi, f)
...
def generate_ir(
    ...
    g_low, g_160, g_2500, g_hi = _graphiceq_points(effect_type, low_eq, high_eq, filter_slider)
